...
if(isset($_POST['btn_save'])){
    ...
    if($id==0){
        $sql="INSERT INTO modell_anyaga (megnevezes) VALUES ('$megnevezes')";
    }
    else{
        $sql="UPDATE modell_anyaga SET megnevezes='$megnevezes' WHERE anyag_id='$id'";
    }
...
if(isset($_POST['del'])){
    $id=(int)$_POST['del'];
    $sql="DELETE FROM modell_anyaga WHERE anyag_id='$id'";
    try{
        mysqli_query($conn,$sql);
        header("Location:index.php?page=".$_POST['page']);
        return;
...
//listázás
$sql="SELECT * FROM modell_anyaga ORDER BY anyag_id";
$result = mysqli_query($conn, $sql);
?>
<div>
    <h3 id="cim">Új modell anyag rögzítése</h3>
...
while($row=mysqli_fetch_assoc($result)){
    $edit="szerkeszt('".$row['anyag_id']."','".$row['megnevezes']."')";
    $del="torol('".$row['anyag_id']."','".$row['megnevezes']."')";
    echo '
    <tr>
        <td>'.$row['anyag_id'].'</td>
