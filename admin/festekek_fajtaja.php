...
if(isset($_POST['btn_save'])){
    ...
    if($id==0){
        $sql="INSERT INTO festekek_fajtaja (megnevezes) VALUES ('$megnevezes')";
    }
    else{
        $sql="UPDATE festekek_fajtaja SET megnevezes='$megnevezes' WHERE fajta_id='$id'";
    }
...
if(isset($_POST['del'])){
    $id=(int)$_POST['del'];
    $sql="DELETE FROM festekek_fajtaja WHERE fajta_id='$id'";
    try{
        mysqli_query($conn,$sql);
        header("Location:index.php?page=".$_POST['page']);
        return;
...
//listázás
$sql="SELECT * FROM festekek_fajtaja ORDER BY fajta_id";
$result = mysqli_query($conn, $sql);
?>
<div>
    <h3 id="cim">Új festék fajta rögzítése</h3>
...
while($row=mysqli_fetch_assoc($result)){
    $edit="szerkeszt('".$row['fajta_id']."','".$row['megnevezes']."')";
    $del="torol('".$row['fajta_id']."','".$row['megnevezes']."')";
    echo '
    <tr>
        <td>'.$row['fajta_id'].'</td>
