...
if(isset($_POST['btn_save'])){
    ...
    if($id==0){
        $sql="INSERT INTO festekek_merete (megnevezes) VALUES ('$megnevezes')";
    }
    else{
        $sql="UPDATE festekek_merete SET megnevezes='$megnevezes' WHERE meret_id='$id'";
    }
...
if(isset($_POST['del'])){
    $id=(int)$_POST['del'];
    $sql="DELETE FROM festekek_merete WHERE meret_id='$id'";
    try{
        mysqli_query($conn,$sql);
        header("Location:index.php?page=".$_POST['page']);
        return;
...
//listázás
$sql="SELECT * FROM festekek_merete ORDER BY meret_id";
$result = mysqli_query($conn, $sql);
?>
<div>
    <h3 id="cim">Új festék méret rögzítése</h3>
...
while($row=mysqli_fetch_assoc($result)){
    $edit="szerkeszt('".$row['meret_id']."','".$row['megnevezes']."')";
    $del="torol('".$row['meret_id']."','".$row['megnevezes']."')";
    echo '
    <tr>
        <td>'.$row['meret_id'].'</td>
