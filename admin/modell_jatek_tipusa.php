...
if(isset($_POST['btn_save'])){
    ...
    if($id==0){
        $sql="INSERT INTO jatek_tipusa (megnevezes) VALUES ('$megnevezes')";
    }
    else{
        $sql="UPDATE jatek_tipusa SET megnevezes='$megnevezes' WHERE tipus_id='$id'";
    }
...
if(isset($_POST['del'])){
    $id=(int)$_POST['del'];
    $sql="DELETE FROM jatek_tipusa WHERE tipus_id='$id'";
    try{
        mysqli_query($conn,$sql);
        header("Location:index.php?page=".$_POST['page']);
        return;
...
//listázás
$sql="SELECT * FROM jatek_tipusa ORDER BY tipus_id";
$result = mysqli_query($conn, $sql);
?>
<div>
    <h3 id="cim">Új modell játék típus rögzítése</h3>
...
while($row=mysqli_fetch_assoc($result)){
    $edit="szerkeszt('".$row['tipus_id']."','".$row['megnevezes']."')";
    $del="torol('".$row['tipus_id']."','".$row['megnevezes']."')";
    echo '
    <tr>
        <td>'.$row['tipus_id'].'</td>
