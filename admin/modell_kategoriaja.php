...
if(isset($_POST['btn_save'])){
    ...
    if($id==0){
        $sql="INSERT INTO modell_kategoriaja (megnevezes) VALUES ('$megnevezes')";
    }
    else{
        $sql="UPDATE modell_kategoriaja SET megnevezes='$megnevezes' WHERE kategoria_id='$id'";
    }
...
if(isset($_POST['del'])){
    $id=(int)$_POST['del'];
    $sql="DELETE FROM modell_kategoriaja WHERE kategoria_id='$id'";
    try{
        mysqli_query($conn,$sql);
        header("Location:index.php?page=".$_POST['page']);
        return;
...
//listázás
$sql="SELECT * FROM modell_kategoriaja ORDER BY kategoria_id";
$result = mysqli_query($conn, $sql);
?>
<div>
    <h3 id="cim">Új modell kategória rögzítése</h3>
...
while($row=mysqli_fetch_assoc($result)){
    $edit="szerkeszt('".$row['kategoria_id']."','".$row['megnevezes']."')";
    $del="torol('".$row['kategoria_id']."','".$row['megnevezes']."')";
    echo '
    <tr>
