...
if(isset($_POST['btn_save'])){
    ...
    if($id==0){
        $sql="INSERT INTO festekek_felulete (megnevezes) VALUES ('$megnevezes')";
    }
    else{
        $sql="UPDATE festekek_felulete SET megnevezes='$megnevezes' WHERE felulet_id='$id'";
    }
...
if(isset($_POST['del'])){
    $id=(int)$_POST['del'];
    $sql="DELETE FROM festekek_felulete WHERE felulet_id='$id'";
    try{
        mysqli_query($conn,$sql);
        header("Location:index.php?page=".$_POST['page']);
        return;
...
//listázás
$sql="SELECT * FROM festekek_felulete ORDER BY felulet_id";
$result = mysqli_query($conn, $sql);
?>
<div>
    <h3 id="cim">Új festék felület rögzítése</h3>
...
while($row=mysqli_fetch_assoc($result)){
    $edit="szerkeszt('".$row['felulet_id']."','".$row['megnevezes']."')";
    $del="torol('".$row['felulet_id']."','".$row['megnevezes']."')";
    echo '
    <tr>
        <td>'.$row['felulet_id'].'</td>
